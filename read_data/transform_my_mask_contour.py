...
def affine(image, shear):
    random_affine = RandomAffine(degrees=0, translate=None, scale=None, shear=shear, interpolation=InterpolationMode.BILINEAR, fill=0)
    return random_affine(image)

# 旋转
def transform_rotate(image, mask, mask_contour):
    ...
    for i in range(len(mask_contour)):
        mask_contour[i] = mask_contour[i].rotate(angle, expand=True)

    # image = tf.to_tensor(image)
    # mask = tf.to_tensor(mask)
    return image, mask, mask_contour

# 错切
def transform_shear(image, mask, mask_contour):
    ...
    if random.random() > 0.5:
        ...
        for i in range(len(mask_contour)):
            mask_contour[i] = affine(mask_contour[i], shear=(angle, angle, 0, 0))
    # 垂直错切
    else:
        ...
        for i in range(len(mask_contour)):
            mask_contour[i] = affine(mask_contour[i], shear=(0, 0, angle, angle))

    return image, mask, mask_contour

# 水平/垂直翻转
def transform_flip(image, mask, mask_contour):
    if random.random() > 0.5:
        image = tf.hflip(image)
        for i in range(len(mask)):
            mask[i] = tf.hflip(mask[i])
        for i in range(len(mask_contour)):
            mask_contour[i] = tf.hflip(mask_contour[i])
    else:
        image = tf.vflip(image)
        for i in range(len(mask)):
            mask[i] = tf.vflip(mask[i])
        for i in range(len(mask_contour)):
            mask_contour[i] = tf.vflip(mask_contour[i])
...
def transform_translate_horizontal(image, mask, mask_contour, scale=0.5):
    # 获取image的长宽
    w, h = image.size
    ...
    if random.random() > 0.5:
        ...
        for i in range(len(mask_contour)):
            mask_contour[i] = tf.crop(mask_contour[i], top=0, left=0, height=h, width=w - w*scale)
            mask_contour_pad.append(tf.pad(mask_contour[i], padding=[0, 0, int(w*scale), 0], fill=0))
        ...
        for i in range(len(mask_contour)):
            mask_contour[i] = tf.crop(mask_contour[i], top=0, left=w * scale, height=h, width=w - w * scale)
            mask_contour_pad.append(tf.pad(mask_contour[i], padding=[int(w * scale), 0, 0, 0], fill=0))

    return image_pad, mask_pad, mask_contour_pad

# 上下平移
def transform_translate_vertical(image, mask, mask_contour, scale=0.5):
    # 获取image的长宽
    w, h = image.size
    ...
    if random.random() > 0.5:
        ...
        for i in range(len(mask_contour)):
            mask_contour[i] = tf.crop(mask_contour[i], top=h * scale, left=0, height=h - h * scale, width=w)
            mask_contour_pad.append(tf.pad(mask_contour[i], padding=[0, int(h * scale), 0, 0], fill=0))
    else:
        ...
        for i in range(len(mask_contour)):
            mask_contour[i] = tf.crop(mask_contour[i], top=0, left=0, height=h - h*scale, width=w)
            mask_contour_pad.append(tf.pad(mask_contour[i], padding=[0, 0, 0, int(h * scale)], fill=0))
